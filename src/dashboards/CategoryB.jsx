...
const colors = [
  ...
  { bg: 'rgba(153, 102, 255, 0.6)', border: 'rgba(153, 102, 255, 1)', grad: 'linear-gradient(90deg,#9966ff 60%,#b39ddb 100%)', box: 'rgba(153,102,255,0.13)', label: 'Compteur Violet' },
  { bg: 'rgba(255, 159, 64, 0.6)', border: 'rgba(255, 159, 64, 1)', grad: 'linear-gradient(90deg,#ff9f40 60%,#ffd180 100%)', box: 'rgba(255,159,64,0.13)', label: 'Compteur Orange' },
...
function CategoryB() {
  ...
  const labelsMinute = Array.from({length: points}, (_, i) => {
    const d = new Date(Date.now() - (points - 1 - i) * windowMs);
    return d.toLocaleTimeString();
  });

  const days = 14; // nombre de jours à afficher
  const labelsDay = Array.from({length: days}, (_, i) => {
    const d = new Date();
    d.setDate(d.getDate() - (days - 1 - i));
    return d.toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit' });
  });

  const lineDataMinute = {
    labels: labelsMinute,
    ...
    <div className="dashboard">
      ...
      <div className="dashboard-row">
        ...
        <div className="dashboard-row-bottom">
          ...
          <div className="graph-container large">
            <Line data={lineDataDay} options={{ plugins: { legend: { labels: { color: colors[0].border }}}}} />
          </div>
        </div>
      </div>
    </div>
